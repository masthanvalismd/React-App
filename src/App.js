...
export default function App() {
  ...
  return (
   ...
   {Movies.map((movie) => (
     <Welcome poster={movie.poster} name={movie.name} rating={movie.rating}  summary={ movie.summary}/>
   ))}
      </div>
   </div>
  );
...
function Welcome({ poster, name, rating, summary }) {
  const styles = {
  ...
  return (
    <div className="Movie">
      ...
      <h1>{name}</h1> 
      <p style={styles} className="rating" >{rating}⭐</p>
      </div>
      <p>{summary}</p>
    </div>
  )
